Shader "Custom/Surface-3D-NURBS-14"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // Global 3D Displacement Function affecting X, Y, and Z
            float3 ApplyStrongDeformation(float3 localPos, float3 worldOrigin, float waveSpeed, float waveAmp)
            {
                ...
                // --- Y Axis Deformation (Sharp Ridge Networks) ---
                ...
                // --- X & Z Axis Deformation (Swirling/Pushing the control lattice) ---
                // We use sine and cosine shifts linked to the opposite coordinates to twist/shear the positions
                ...
                // Return the completely transformed position vector
            ...
            d2f ds(HS_CONSTANT_OUTPUT input, float2 uvDomain : SV_DomainLocation, const OutputPatch<v2h, 4> patch)
            {
                ...
                // Evaluate the 4x4 control point surface matrix
                for (int z = 0; z < 4; ++z) {
                    float bz = Bernstein(z, uv.y);
                    for (int x = 0; x < 4; ++x) {
                        float bx = Bernstein(x, uv.x);
                        
                        // Default undeformed baseline coordinate for this control point index
                        float3 cpPos = float3((float)x * 1.5f - 2.25f, 0.0f, (float)z * 1.5f - 2.25f);
                        
                        // Assign original rational configuration weights
                        float weight = 1.0f;
                        if ((x == 1 || x == 2) && (z == 1 || z == 2)) {
                            weight = 2.0f;
                        ...
                        // FIX: All control points (0 through 3) are now fully processed through the 3D math 
                        cpPos = ApplyStrongDeformation(cpPos, worldOrigin, _WaveSpeed, _WaveAmp);
                ...
                // Rational perspective projection division
            ...
            {
                float3 colorA = float3(0.0f, 0.8f, 0.9f); // Electric Cyan
                float3 colorB = float3(0.6f, 0.1f, 0.8f); // Neon Purple
